...
import com.adobe.cq.testing.selenium.pagewidgets.granite.KeyboardShortcuts;
...
import com.adobe.cq.testing.selenium.pagewidgets.granite.shell.*;
...
public class ShellPage extends BasePage {
    ...
    /**
     * Open the dialog using the keyboard.
     *
     * @return {KeyboardShortcutsComponent}
     */
    public KeyboardShortcuts openKeyboardShortcutsOverlayWithShortcutKey() {
        actions().sendKeys(Keys.SHIFT, Keys.getKeyFromUnicode('?')).perform();
        KeyboardShortcuts shortcutsOverlay = new KeyboardShortcuts();
        shortcutsOverlay.waitVisible();
        return shortcutsOverlay;
    }

    /**
     * Close the dialog using the keyboard.
     */
    public void closeKeyboardShortcutsOverlayWithShortcutKey() {
    ...
    /**
     * Open the user help drop-down.
     *
     * @return ShellHelpComponent.
     */
    public ShellHelp openUserHelp() {
        ShellHeaderHelpIcon helpIcon = new ShellHeaderHelpIcon();
        return helpIcon.open();
    }

    /**
     * Open user properties by clicking on the user icon.
     *
     * @return UserPropertiesComponent.
     */
    public UserProperties openUserProperties() {
        ShellHeaderUserIcon userIcon = new ShellHeaderUserIcon();
        return userIcon.open();
    }

    /**
     * Open user preferences dialog by clicking on user and then the preferences link.
     *
     * @return UserPreferencesComponent.
     */
    public UserPreferences openUserPreferences() {
        ShellHeaderUserIcon userIcon = new ShellHeaderUserIcon();
        UserProperties userProp = userIcon.open();
        ShellUser shellUser = userProp.user();
        ShellUserSubheading shellUserSubHeading = shellUser.subheading();
        ShellUserPreferencesLink userPrefLink = shellUserSubHeading.getUserPreferencesLink();
        return userPrefLink.open();
    }

    /**
     * Logout by clicking on the user icon and the logout link.
     *
     * @return ShellPage.
     */
    public ShellPage logout() {
        ShellHeaderUserIcon userIcon = new ShellHeaderUserIcon();
        UserProperties userProp = userIcon.open();
        userProp.signOut();
